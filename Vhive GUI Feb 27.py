...
# #### FIXME: Needs board location information. Function for extend/retract. Function for changing the value of dx on every up/down click (0.5" interval). Include the RESET button inside this function to zero out the number.
...
def login():
    ...
    im = Image.open("bg_gradient1.png")
    bg = ImageTk.PhotoImage(im) 
    canvas = Canvas(w, width=800, height=450)
    canvas.place(x=0, y=0)
    canvas.create_image(0, 0, anchor='nw', image=bg)
    Frame(w,width=250,height=320,bg='white').place(x=50,y=50)

    l0=Label(w,text="V-Hive Green Box", bg='white')
    l0.config(font=ft2)
    l0.place(x=70,y=65)
    ...
    canvas = Canvas(w, width=90, height=80, bg="white", highlightthickness=0, relief='ridge')
    ...
    Button(w,width=20,height=2,fg="white", bg="#5b9aa0", border=0, command=cmd, text="L O G I N").place(x=100,y=390)
